...
public class EditSongActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        confirmEditSong = findViewById(R.id.confirmEditSong);
        editSong = findViewById(R.id.editSong_edittext);
        toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        s = getIntent().getStringExtra("Idstringtekst");
        editSong.setHint(s.subSequence(s.indexOf(" ")+1,s.length()));

        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        String pom = preferences.getString("at","");
        apiServiceEdit = ApiUtils.getApiService(pom);

        confirmEditSong.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (TextUtils.isEmpty(editSong.getText().toString()))
                {
                    ...
                    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            ...
                            String name = editSong.getText().toString().trim();
                            editSongs(pom, name);
                        }
                    });
    ...
    public void editSongs (int id, String name){

        apiServiceEdit.editSongs(id, name).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {

                if(response.isSuccessful()) {
                    Log.i(TAG, "edit submitted to API." + response.code() + " " + response.message());
                    Intent intent = new Intent(EditSongActivity.this,SongsActivity.class);
